...
class TagService:
    ...
    #                 {"id": tag.id, "name": tag.name, "diary_count": diary_count}
    #             )
    #         else:
    #             unused_tags += 1
    #
    #     # 인기 태그 TOP 10
    #     popular_tags = sorted(
    #         tag_usage_stats, key=lambda x: x["diary_count"], reverse=True
    #     )[:10]
    #
    #     # 평균 사용량
    #     avg_usage = (
    #         sum(t["diary_count"] for t in tag_usage_stats) / len(tag_usage_stats)
    #         if tag_usage_stats
    #         else 0
    #     )
    #
    ...
    #     print(
    #         f"[FORCE DELETE] Admin {admin_user_id} deleting tag '{tag.name}' (used in {diary_count} diaries)"
    #     )
    #
    #     await repository.delete_tag(tag)
    #     return True
    ...
    #     source_tag_id: int, target_tag_id: int, admin_user_id: int
    # ) -> bool:
    #     """
    #     태그 병합 기능
    ...
    #         current_tag_names = [
    #             getattr(tag, "name", "") for tag in getattr(diary, "tags", [])
    #         ]
    #
    #         # source 태그 제거
    ...
    #
    #     print(
    #         f"[TAG MERGE] Admin {admin_user_id} merged '{source_tag.name}' "
    #         f"-> '{target_tag.name}' ({len(source_diaries)} diaries affected)"
    #     )
    #
    #     return True
